...
impl From<[u8; 16]> for PpcVector {
    fn from(x: [u8; 16]) -> Self {
        PpcVector(u128::from_le_bytes(x))
    }
}

impl From<PpcVector> for u128 {
    fn from(x: PpcVector) -> Self {
        x.0
    }
}

// Godbolt link of hand-unrolling: https://godbolt.org/z/Yh39od
impl From<PpcVector> for [f32; 4] {
    fn from(x: PpcVector) -> Self {
        let bytes = x.0.to_be_bytes();
